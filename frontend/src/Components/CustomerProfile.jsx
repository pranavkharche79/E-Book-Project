...
const CustomerProfile = ({ customerId }) => {
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/customers/${customerId}`); // Adjust the URL to match your backend API
    ...
    <div className="container mt-5">
      <div className="row">
        <div className="col-md-4">
          <img src="profile-picture-url.jpg" alt="Profile" className="img-fluid rounded-circle mb-3" />
        </div>
        <div className="col-md-8">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {editing ? (
              ...
              <button type="button" className="btn btn-info" onClick={() => setEditing(true)}>Edit</button>
            )}
          </form>
        </div>
      </div>
        ...
        <div className="col">
          ...
          <ul>
            {customer.alist.map(address => (
              <li key={address.id}>
                <p><strong>Address:</strong> {address.adrr}</p>
